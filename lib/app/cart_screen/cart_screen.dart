...
class _CartScreenState extends State<CartScreen> {
  bool _showEnvironmentNote = true;
  bool _showTotalSavedNote = true;

  bool _isPolicyAccepted = false; // لحفظ حالة المربع
  bool _isButtonEnabled = false; // لحفظ حالة الزر

  @override
  void initState() {
    super.initState();

    // تأكد من أن الزر غير مفعل في البداية
    _isButtonEnabled = false;

    // إخفاء الويجيتات بعد 5 ثوانٍ
    Future.delayed(Duration(seconds: 5), () {
      setState(() {
        _showEnvironmentNote = false;
        _showTotalSavedNote = false;
      });
    });
  }

  void _onPolicyChanged(bool? newValue) {
    setState(() {
      _isPolicyAccepted = newValue ?? false;
      // تمكين أو تعطيل الزر بناءً على حالة المربع
      _isButtonEnabled = _isPolicyAccepted;
    });
  }

// توليد رقم عشوائي
  double generateEcoScore() {
    final random = Random();
    // توليد رقم عشري بين 0.000001 و 0.000005
    return (1 + random.nextInt(5)) / 1000000;
  }

  Widget buildEnvironmentNote(double ecoScore) {
    return Positioned(
      top: 20,
      left: 20, // وضعها في الزاوية اليسرى العليا
      right: 20, // إضافة right لتحسين العرض في الاتجاهين
      child: Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
          color: Colors.green[50],
          borderRadius:
              BorderRadius.circular(16), // زيادة الانحناء لإعطاء الشكل الستيكي
        ...
        child: Row(
          children: [
            Icon(Icons.eco, color: Colors.green[800], size: 28),
            const SizedBox(width: 10),
            Expanded(
              child: Text(
                "Well done! You contributed to protecting the environment with"
                        .tr +
                    " ${ecoScore.toStringAsFixed(6)} 🌍",
                // 'أحسنت! ساهمت في الحفاظ على البيئة بـ ${ecoScore.toStringAsFixed(6)} 🌍',
                style: TextStyle(
  ...
  Widget buildTotalSavedNote(double totalSaved) {
    return Positioned(
      top: 100, // وضعها أسفل الويجيت الأولى في الجزء العلوي
      left: 20,
      right: 20,
      child: Container(
        padding: const EdgeInsets.all(16),
            ...
            Expanded(
              child: Text(
                "You saved".tr +
                    ...
                    "EGP from discounts".tr,

                // 'لقد وفّرت ${totalSaved.toStringAsFixed(2)} جنيه من الخصومات 🎉',
                style: TextStyle(
                  fontSize: 16, // تكبير الخط
                  fontWeight: FontWeight.bold,
  ...
  Widget build(BuildContext context) {
    double ecoScore = generateEcoScore();

    final themeChange = Provider.of<DarkThemeProvider>(context);
    double totalSaved = 0;

    cartItem.forEach((cartItemModel) {
      double originalPrice = double.tryParse(cartItemModel.price ?? "0") ?? 0;
      double discountPrice =
          double.tryParse(cartItemModel.discountPrice ?? "0") ?? 0;
      double saved = originalPrice - discountPrice;

      totalSaved += saved;

      print('Item: ${cartItemModel.name ?? 'No Title'}');
    ...
    print('Total saved on all items: $totalSaved');
            ...
            body: cartItem.isEmpty
                ...
                : Stack(
                    children: [
                              ...
                              child: Column(
                                children: [
                                  TextFieldWidget(
                                    ...
                                    hintText:
                                        'Write remarks for the restaurant'.tr,
                                    maxLine: 4,
                                  ),
                                ],
                              ),
                      ...
                      if (_showEnvironmentNote)
                        buildEnvironmentNote(ecoScore), // عرض ملاحظة البيئة
                      if (_showTotalSavedNote)
                        buildTotalSavedNote(totalSaved), // عرض ملاحظة التوفير
                    ],
                  ),
                        ...
                        child: Column(
                          children: [
                            ...
                            Row(children: [
                              ...
                              Text(
                                "I agree to the Privacy Policy and Terms".tr,
                                style: TextStyle(
                                  fontSize: 14,
                                  color: themeChange.getThem()
                                      ? Colors.white
                            ...
                            Row(
                              children: [
                                ...
                                // زر الدفع الذي يعتمد على حالة المربع
                                ...
                                Expanded(
                                  child: RoundedButtonFill(
                                    ...
                                    fontSizes: 16,
                                    onPress: () async {
                                      // هنا سيكون منطق الدفع الموجود بالفعل في الكود
                                      ...
                                      if ((controller.specialDiscountAmount
                                                  .value >=
                                              1) &&
                                          (controller
